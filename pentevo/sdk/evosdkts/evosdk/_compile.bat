...
rem echo evoresc "%temp%\out.ihx" "..\evosdk\startup.bin" "%soundfx%" "%temp%\music.lst" "%temp%\palette.lst" "%temp%\image.lst" "%temp%\sample.lst" "%temp%\sprite.lst"
...
if not defined outspg goto nospg:
if not defined spgpack (
spgbld -b spg.ini %outspg% >nul
) else (
spgbld -b spg.ini %outspg% -c %spgpack% >nul
)
...
if defined outspg copy %temp%\%outspg% >nul
