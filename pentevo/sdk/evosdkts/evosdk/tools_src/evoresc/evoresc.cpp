...
#define SPRBUF_PAGE	12	//4 страницы
#define SPRTBL_PAGE	6	//параметры спрайтов

#define CC_PAGE0	20	//код
#define CC_PAGE1	21
#define CC_PAGE2	22
#define CC_PAGE3	23

#define SND_PAGE	0	//код звука и эффекты
#define PAL_PAGE	1	//палитры

#define GFX_PAGE	24	//графика начинается с фиксированной страницы

#define PACK_PIXELS(col1,col2) (((col1)<<4)|(col2))
...
bool page_save(int slot,int page)
{
	...
	fprintf(fileSPGini,"Block = #C000,%i,page_%i.bin\n",page,page);
...
inline void convert_tile(unsigned char* tile,int tx,int ty)
{
	...
	for(i=0;i<8;++i)
	{
		...
		tile[pp+1]=PACK_PIXELS(chr[i][2],chr[i][3]);
		tile[pp+2]=PACK_PIXELS(chr[i][4],chr[i][5]);
		tile[pp+3]=PACK_PIXELS(chr[i][6],chr[i][7]); 
		pp+=4;
	}
}
...
int main(int argc,char* argv[])
{
	...
	fileSPGini=fopen("_temp_/spg.ini","wt");
	...
	fprintf(fileSPGini,"Desc = EVO SDK\n");
	fprintf(fileSPGini,"Start = 0xe000\n");
	fprintf(fileSPGini,"Stack = $dfff\n");
	fprintf(fileSPGini,"Resident = #5B00\n");
	fprintf(fileSPGini,"Page3 = %i\n",CC_PAGE3);
	fprintf(fileSPGini,"Clock = 2\n");
	fprintf(fileSPGini,"INT = 1\n");
	fprintf(fileSPGini,"Pager = 0\n");
