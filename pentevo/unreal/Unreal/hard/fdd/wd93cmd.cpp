...
void WD1793::trdos_traps()
{
	...
	if (pc == 0x3DFD && bankr[0][0x3DFD] == 0x3E && bankr[0][0x3DFF] == 0x0E)
	{
		...
		cpu.pc |= *cpu.direct_mem(cpu.sp++) << 8;
		cpu.a = 0;
		cpu.c = 0;
	}
	...
	{
		cpu.a = cpu.c = 1; return;
	} // no delays
	if (pc == 0x3FEC && bankr[0][0x3FED] == 0xA2 && (state == S_READ || (state2 == S_READ && state == S_WAIT)))
	{
		trdos_load = romled_time;
		if (rqs & DRQ) {
			*cpu.direct_mem(cpu.hl) = data; // move byte from controller
			cpu.hl++;
			cpu.b--;
			rqs &= ~DRQ; status &= ~WDS_DRQ;
		}
		...
		if (seldrive->t.trkd)
		{
			while (rwlen) { // move others
				*cpu.direct_mem(cpu.hl) = seldrive->t.trkd[rwptr++];
				rwlen--;
				cpu.hl++; cpu.b--;
			}
		}
	...
	if (pc == 0x3FD1 && bankr[0][0x3FD2] == 0xA3 &&
		...
		trdos_save = romled_time;
		while (rwlen > 1) {
			seldrive->t.write(rwptr++, *cpu.direct_mem(cpu.hl), 0);
			rwlen--;
			cpu.hl++; cpu.b--;
		}
		cpu.pc += 2; // skip OUTI
