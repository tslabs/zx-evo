...
void joystick_init()
{
  ...
  if (cfg_joy_available())
  {
    // use keymap from EEPROM TLV config
    joystick_flags |= JOYSTICK_FLAG_EEPROM_MAP;

    // read config from EEPROM TLV config
    joystick_mode.raw = cfg_joy_read(CFG_JOY_MODE);

    // read autofire map
    for (u8 pos = 0; pos < sizeof(joystick_autofire_mask); pos++)
    {
      joystick_autofire_mask[pos] = cfg_joy_read(CFG_JOY_PAD0_AUTOFIRE + pos);
    }
  }
...
void joystick_set_mode(u8 mode)
{
  ...
  // write mode to EEPROM TLV config
  if ((joystick_flags & JOYSTICK_FLAG_EEPROM_MAP) == 0)
  {
    cfg_joy_init(joystick_mode.raw);
    joystick_flags |= JOYSTICK_FLAG_EEPROM_MAP;
  }
  else
  {
    cfg_joy_write(CFG_JOY_MODE, joystick_mode.raw);
  }
...
u8 joystick_keymap_get(u8 joystick, u8 index)
{
  ...
  {
    // read from EEPROM TLV config
    u8 tblptr = index + (joystick == 1 ? CFG_JOY_PAD1_MAP : CFG_JOY_PAD0_MAP);
    ret = cfg_joy_read(tblptr);
  }
  else
  {
    // read default keymap
...
{
  // sanity check
  if ((index > 15) || (joystick > 1) || ((joystick_flags & JOYSTICK_FLAG_EEPROM_MAP) == 0)) return CFGIF_PAD_MAPPING_NO_KEY;
...
{
  // sanity check
  if ((index > 1) || (joystick > 1) || ((joystick_flags & JOYSTICK_FLAG_EEPROM_MAP) == 0)) return 0;
...
void joystick_autofire_write(u8 joystick, u8 index, u8 data)
{
  ...
  // write
  u8 tblptr = index + (joystick == 1 ? CFG_JOY_PAD1_AUTOFIRE : CFG_JOY_PAD0_AUTOFIRE);
  cfg_joy_write(tblptr, data);
