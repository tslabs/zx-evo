...
TSF_RESULT tsf_search(TSF_VOLUME *vol, u32 *chunk_addr, const char *name)
{
  ...
  memcpy(search_name, name, name_len);
  ...
  for (u32 i = 0; i < cfg->bulk_size; i += cfg->block_size)
  {
    ...
    if (fnlen)
    {
      ...
      if (memcmp(search_name, cfg->buf, fnlen))
        continue;
    }
...
TSF_RESULT tsf_delete_chunk_range(TSF_VOLUME *vol, u32 addr, u32 blocks)
{
  TSF_RESULT rc;
...
TSF_RESULT tsf_delete(TSF_VOLUME *vol, const char *name)
{
  ...
  do
  {
    ...
    {
      rc = tsf_delete_chunk_range(vol, range_addr, range_blocks);
      if (rc != TSF_RES_OK)
        return rc;
